...
| [`fulfillment`](fulfillment/) | Vendor-side triage of inbound fulfillment requests (which lifecycle transition, and why) and distributor/reseller-side subscription operations (inspect, then change / cancel / suspend / resume / renew / adjust / transfer). Skills: `connect-request-triage`, `connect-subscription-ops`. |
