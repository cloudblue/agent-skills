...
So: grant MCP plus the modules the user actually intends to work with — or the blanket **All modules** grant (`MD-0000`), which resolves to every module and is the simplest token for someone who should see the whole catalog. The tools visible in the catalog reflect the token's module permissions — a smaller-than-expected catalog usually means a narrower-than-expected token.
