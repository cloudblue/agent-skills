...
description: Use when the user needs to connect an AI agent to the CloudBlue Connect MCP server for the first time, or to fix a Connect MCP connection that stopped working — configuring the MCP client, creating a Connect API token with the right permissions, or diagnosing errors. Triggers on requests like "set up Connect MCP", "connect Claude to CloudBlue Connect", "configure the Connect MCP endpoint", "my Connect token gets 403", "401 from Connect", "no Connect tools show up", or "the Connect MCP server isn't working".
version: 0.1.0
license: Apache-2.0
metadata:
  hermes:
...
at a path like `/public/v1/mcp/usage/`, it is stale — replace it with the
single endpoint above.
