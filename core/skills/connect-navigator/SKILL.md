...
- **Tier levels count upward from the end customer.** **T1** is the account directly serving the end customer (typically a reseller); **T2** sits one level above T1 (a reseller selling through T1). The distributor or provider operating the marketplace is **T0** and is not itself a tier account. How many levels a channel uses is a setup decision — many use T1 only, so "the tier account" in conversation almost always means T1.
